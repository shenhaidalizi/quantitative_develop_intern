...
public:
	...
	void ReqParkedOrderInsert()
	{
		...
		strcpy_s(a.UserID, g_chUserID);
		OrderRef_num++;
		snprintf(a.OrderRef, sizeof(a.OrderRef), "%d", OrderRef_num);
	...
	void ReqOrderInsert_dce()
	{
		...
		OrderRef_num++;
		snprintf(ord.OrderRef, sizeof(ord.OrderRef), "%d", OrderRef_num);
	...
	cir1:int bankid;
		...
		{
			//strcpy_s(a.BankID, snprintf(a.BankID, sizeof(a.BankID), "%d", bankid));///银行代码
			snprintf(a.BankID, sizeof(a.BankID), "%d", bankid);
		}
		else
		{
			LOG("请重新输入银行代码。\n");
