...
#include <stdio.h>
#include "td_hook.h"
#define CTP_NO_USER_SYSINFO 1
static td_hook_fn g_td_hook = nullptr;
extern "C" void td_set_hook(td_hook_fn fn) { g_td_hook = fn; }
...
void CTraderSpi::OnRspUserLogin(CThostFtdcRspUserLoginField* pRspUserLogin, CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	...
	LOG("\tbIsLast [%d]\n", bIsLast);
	LOG("</OnRspUserLogin>\n");
	if (g_td_hook) g_td_hook("Login", "", "", (pRspInfo && pRspInfo->ErrorID) ? "Fail" : "OK");
};
...
void CTraderSpi::OnRspOrderInsert(CThostFtdcInputOrderField* pInputOrder, CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	...
	LOG("\tbIsLast [%d]\n", bIsLast);
	LOG("</OnRspOrderInsert>\n");
	if (g_td_hook && pInputOrder) g_td_hook("OrderInsert", pInputOrder->OrderRef, pInputOrder->InstrumentID,
		(pRspInfo && pRspInfo->ErrorID) ? "Fail" : "OK");
};
...
void CTraderSpi::OnRspOrderAction(CThostFtdcInputOrderActionField* pInputOrderAction, CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	...
	LOG("\tbIsLast [%d]\n", bIsLast);
	LOG("</OnRspOrderAction>\n");
	if (g_td_hook && pInputOrderAction) g_td_hook("OrderAction", pInputOrderAction->OrderRef, pInputOrderAction->InstrumentID,
		(pRspInfo && pRspInfo->ErrorID) ? "Fail" : "OK");
};
...
void CTraderSpi::OnRtnOrder(CThostFtdcOrderField* pOrder)
{
	...
	LOG("</OnRtnOrder>\n");
	if (g_td_hook && pOrder) { char buf[128]; std::snprintf(buf,sizeof(buf),"Status=%c Traded=%d",
		pOrder->OrderStatus, pOrder->VolumeTraded);
		g_td_hook("OnRtnOrder", pOrder->OrderRef, pOrder->InstrumentID, buf); }
};
void CTraderSpi::OnRtnTrade(CThostFtdcTradeField* pTrade)
{
	...
	LOG("</OnRtnTrade>\n");
	if (g_td_hook && pTrade) { char buf[128]; std::snprintf(buf,sizeof(buf),"Px=%.10f Vol=%d",
		pTrade->Price, pTrade->Volume);
		g_td_hook("OnRtnTrade", pTrade->OrderRef, pTrade->InstrumentID, buf); }
};
