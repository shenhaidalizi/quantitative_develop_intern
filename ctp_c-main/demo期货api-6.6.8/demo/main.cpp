...
int main()
{
	...
	while (true)
	{
		...
		case 2:
		{
			...
			g_md_front_str = g_chFrontMdaddr;

			CThostFtdcMdApi  *pUserMdApi = CThostFtdcMdApi::CreateFtdcMdApi("./flow/");
			CSimpleMdHandler ash(pUserMdApi);
			pUserMdApi->RegisterSpi(&ash);
			pUserMdApi->RegisterFront(const_cast<char *>(g_md_front_str.c_str()));
			pUserMdApi->Init();
		...
		cir:pUserApi->RegisterSpi(&sh);
			...
			pUserApi->RegisterFront(const_cast<char *>(g_td_front_str.c_str()));
			pUserApi->Init();
